...
MAX_POSITION_RATIO = 0.5  # 最大仓位比例 (50%)，保留50%底仓
...
class TradingConfig:
    ...
    TREND_OUTPUT_DIR = TREND_OUTPUT_DIR
        ...
        self.GRID_PARAMS = {
            ...
            'volatility_threshold': {
                'ranges': [
                    ...
                    # {'range': [0.60, 0.80], 'grid': 2.5},  # 波动率 60-80%，网格2.5%
                    # 波动率和网格配置说明：
                    # 假设以100元买入，根据不同波动率区间的网格大小，卖出价格和利润如下：
                    # 波动率 0-20%: 网格0.3%, 卖出价格=100.3元, 利润=0.3%
                    # 波动率 20-40%: 网格0.5%, 卖出价格=100.5元, 利润=0.5%
                    # 波动率 40-60%: 网格0.8%, 卖出价格=100.8元, 利润=0.8%
                    # 波动率 60-80%: 网格1.5%, 卖出价格=101.5元, 利润=1.5%
                    # 波动率 80-100%: 网格3.0%, 卖出价格=103.0元, 利润=3.0%
                    # 波动率 100-120%: 网格3.5%, 卖出价格=103.5元, 利润=3.5%
                    # 波动率 >120%: 网格4.0%, 卖出价格=104.0元, 利润=4.0%

                    {'range': [0, 0.20], 'grid': 0.3},     # 波动率 0-20%，网格0.3%（大幅降低以增加成交频率）
                    {'range': [0.20, 0.40], 'grid': 0.5},  # 波动率 20-40%，网格0.5%（大幅降低以增加成交频率）
                    {'range': [0.40, 0.60], 'grid': 0.8},  # 波动率 40-60%，网格0.8%（大幅降低以增加成交频率）
                    {'range': [0.60, 0.80], 'grid': 1.5},  # 波动率 60-80%，网格1.5%（降低以增加成交频率）
                    {'range': [0.80, 1.00], 'grid': 3.0},  # 波动率 80-100%，网格3.0%（保持原值）
                    {'range': [1.00, 1.20], 'grid': 3.5},  # 波动率 100-120%，网格3.5%（保持原值）
                    {'range': [1.20, 999], 'grid': 4.0}    # 波动率 >120%，网格4.0%（保持原值）
                ]
            }
        }
        self.SYMBOL = SYMBOL
